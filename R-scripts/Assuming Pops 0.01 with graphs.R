...
# Clinical data
clinical_data <- c(6512, 799, 462, 4088)

# Constants for mosaic high and aneuploid categories
constant_mosaic_high <- 462
...
# Define the threshold for comparison (e.g., 0.01)
threshold <- 0.01
...
# Identify indices for percentages in the 9.5% to 12.5% range
indexes_in_range <- which(percentages >= 9.5 & percentages <= 12.5)
...
line_color <- "#0073C2FF"  # Blue
highlight_color <- "#D55E00"  # Orange
hline_color <- "#D55E00"  # Bright Orange for p-value threshold

# Create the main plot
assumedpops <- ggplot(data, aes(x = Percentage, y = P_Value)) +
  geom_line(color = line_color) +
  geom_point(color = line_color) +
  geom_vline(xintercept = matching_percentage, linetype = "dashed", color = highlight_color) +
  geom_vline(xintercept = drop_off_percentage, linetype = "dashed", color = highlight_color) +
  geom_hline(yintercept = threshold, linetype = "dashed", color = hline_color) +
  annotate("text", x = matching_percentage, y = max(p_values) * 0.75, label = paste(matching_percentage, "%"), 
           color = "black", vjust = -1, hjust = 1.3, size = 5) +  # Increase text size
  annotate("text", x = drop_off_percentage, y = max(p_values) * 0.85, label = paste(drop_off_percentage, "%"), 
           color = "black", vjust = -1, hjust = -0.3, size = 5) +  # Increase text size
  annotate("text", x = max(percentages) * 0.85, y = threshold, label = "p = 0.01", 
           color = "black", hjust = 1.1, vjust = -0.5, size = 5) +  # Increase text size
  ...
  theme(panel.grid = element_blank(),
        ...
        plot.title = element_text(size = 16),  # Title text
...
zoom_plot <- ggplot(data, aes(x = Percentage, y = P_Value)) +
  ...
  geom_hline(yintercept = threshold, linetype = "dashed", color = hline_color) +
  labs(x = "Percentage of Mosaic Low", y = "P-value") +
  scale_x_continuous(limits = c(matching_percentage - 1, drop_off_percentage + 1)) +  # Zoom window
...
# Create a combined plot using ggdraw
combined_plot_0.01 <- ggdraw() +
  draw_plot(assumedpops) +
  draw_plot(zoom_plot, x = 0.38, y = 0.35, width = 0.5, height = 0.5)  # Adjusted position and size of the inset

# Print the combined plot
print(combined_plot_0.01)

# Save the combined plot to a file with a white background
ggsave("combined_plot_0.01.png", plot = combined_plot_0.01, width = 6.7, height = 6.7, dpi = 300, bg = "white")
