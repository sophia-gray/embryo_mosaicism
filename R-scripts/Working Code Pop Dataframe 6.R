...
perform_chi_squared_test <- function(expected_counts, observed_counts) {
...
for (i in 1:nrow(populations_df)) {
  ...
  # Convert category counts to a named list
  ...
  chi_squared_result <- perform_chi_squared_test(expected_counts, observed_counts)
