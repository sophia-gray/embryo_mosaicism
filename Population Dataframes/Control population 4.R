...
# Choose the population to visualize (Population 4)
population_index <- 4  # Population 4
...
# Create a data frame for Population 4
pop_df <- data.frame(
...
# Plot the histogram for Population 4
plot4 <- ggplot(pop_df, aes(x = Category, y = Count, fill = Category)) +
  ...
  scale_fill_manual(values = c("Euploid" = "#E69F00", "Mosaic Low" = "#56B4E9",
                    ...
                    name = "Control Population 4") +  # Set legend title
  theme(
...
ggsave(filename = paste0("histogram_control_pop4.png"), plot = plot4, bg = "white", width = 6.7, height = 6.7, dpi = 300)
