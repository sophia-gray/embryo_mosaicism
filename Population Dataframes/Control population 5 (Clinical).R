...
set_populations <- list(
  ...
  c(6512, 799, 462, 4088)     # Population 5 (same as clinical data)
)
...
# Plot the histogram for Population 5
plot5 <- ggplot(pop_df, aes(x = Category, y = Count, fill = Category)) +
  ...
  scale_fill_manual(values = c("Euploid" = "#E69F00", "Mosaic Low" = "#56B4E9",
                    ...
                    name = "Control Population 5") +  # Set legend title
  theme(
...
ggsave(filename = paste0("histogram_control_pop5.png"), plot = plot5, bg = "white", width = 6.7, height = 6.7, dpi = 300)
...
# Assuming you have saved each plot as 'plot1', 'plot2', etc.
combined_control_plot <- (plot1 | plot2) / (plot3 | plot4) + plot_annotation(tag_levels = "A")

# Save the combined plot as a PNG file
ggsave("combined_histogram_controls.png", combined_control_plot, width = 6.7, height = 6.7, dpi = 300)
