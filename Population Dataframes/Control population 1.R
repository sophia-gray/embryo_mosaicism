...
# Choose the population to visualize (Population 1)
population_index <- 1  # Population 1
...
# Create a data frame for Population 1
pop_df <- data.frame(
...
# Plot the histogram for Population 1
plot1 <- ggplot(pop_df, aes(x = Category, y = Count, fill = Category)) +
  ...
  scale_fill_manual(values = c("Euploid" = "#E69F00", "Mosaic Low" = "#56B4E9",
                    ...
                    name = "Control Population 1") +  # Set legend title
  theme(
...
ggsave(filename = paste0("histogram_control_pop1.png"), plot = plot1, bg = "white", width = 6.7, height = 6.7, dpi = 300)
