...
# Extract corresponding populations and p-values
populations_in_range <- intermediate_populations[indexes_in_range]
percentages_in_range <- percentages[indexes_in_range]
p_values_in_range <- p_values[indexes_in_range]  # Extract p-values for the same indices

# Create a data frame with the population details and p-values
pops9.512.5_df <- data.frame(
  ...
  P_Value = p_values_in_range  # Add p-values column
)
