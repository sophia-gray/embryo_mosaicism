#Code for assumed simulated populations 
...
assumedpops <- ggplot(data, aes(x = Percentage, y = P_Value)) +
  ...
  annotate("text", x = start_percentage, y = max(p_values) * 0.75, label = "9.5%", 
           color = highlight_color, vjust = -1, hjust = 1.3) +
  annotate("text", x = end_percentage, y = max(p_values) * 0.85, label = "12.5%", 
