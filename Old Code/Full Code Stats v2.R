...
# Assuming combined_results is already prepared with appropriate columns

# Updated clinical data with changed categories and counts
clinical_data <- data.frame(
  clinical_category = c("euploid", "mosaic_low", "mosaic_high", "aneuploid"),
  counts = c(6512, 799, 462, 4088)
)

# Prepare expected proportions from clinical data
clinical_data_for_merge <- clinical_data %>%
  pivot_wider(names_from = clinical_category, values_from = counts)

# Function to determine p-value based on observed counts
compute_p_value <- function(x) {
  # Check if all counts are zero for this population
  if (all(x == 0)) {
    return(NA)  # Set p-value to NA if all counts are zero
  } else {
    # Calculate expected proportions from clinical data
    expected_proportions <- unlist(clinical_data_for_merge) / sum(unlist(clinical_data_for_merge))
    # Calculate chi-squared statistic (not actually performing the test, just computing the statistic)
    chi_sq_stat <- sum((x - expected_proportions * sum(x))^2 / (expected_proportions * sum(x)))
    # Degrees of freedom (number of categories - 1)
    df <- length(x) - 1
    # Calculate p-value based on chi-squared statistic (assuming chi-squared distribution)
    p_value <- 1 - pchisq(chi_sq_stat, df)
    return(p_value)
  }
}

# Calculate p-values for each population
p_values <- combined_results %>%
  mutate(p_value = apply(select(., euploid, mosaic_low, mosaic_high, aneuploid), 1, compute_p_value)) %>%
