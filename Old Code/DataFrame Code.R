...
# Plot the combined results using ggplot2
ggplot(combined_results, aes(x = aneuploid_counts, fill = as.factor(simulation_id))) +
  # Create a histogram with binwidth of 1, black borders, and alpha transparency
  geom_histogram(binwidth = 1, color = "black", alpha = 0.7) +
  
  # Create separate plots for each simulation ID
  facet_wrap(~ simulation_id, scales = "free_y") +
  
  # Add labels and adjust theme
  labs(y = "Frequency", fill = "Simulation ID") +
  theme_bw() +
  theme(
