...
p_values <- vector("numeric", length = nrow(populations_df))

# Loop through each population
for (i in 1:nrow(populations_df)) {
  # Observed counts (number of aneuploid cells)
  observed_counts <- combined_results$aneuploid_counts[combined_results$population_id == populations_df$population_id[i]]
