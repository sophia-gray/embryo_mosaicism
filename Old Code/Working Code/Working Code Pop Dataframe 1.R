...
perform_chi_squared_test <- function(observed_counts, expected_counts) {
...
for (i in 1:nrow(populations_df)) {
  ...
  # Define reference data for the chi-squared test (example)
  reference_data <- c(
    euploid = 6512,
    mosaic_low = 799,
    mosaic_high = 462,
  ...
  chi_squared_result <- perform_chi_squared_test(observed_counts, expected_counts)
  ...
  saveRDS(population_results, paste0("Population_", population_row$PopulationID, ".RDS"))
...
# Print final message
cat("Final Results:\n")
print("Results saved to individual RDS files and summary_results.csv")
